 ...
 * To avoid STARVATION:
 * If 5 cars in a row in the same direction advance on bridge and there are cars in the other direction,
 * then cars in the other direction will be allowed to advance.
 */
...
int cars_since_direction_change = 0;
...
int main(int argc, char **argv) {
    ...
    current_direction = cars[0].direction;
    next_direction = cars[0].direction;
    for (i = 0; i < car_count; i++) {
        if (cars[i].arrival < earliest_arrival) {
            earliest_arrival = cars[i].arrival;
            current_direction = cars[i].direction;
            next_direction = cars[i].direction;
        }
    }
    ...
    // All threads complete, destroy all semaphores
...
// Function to be executed by car threads
void *car_thread_execute(void *car_ptr) {
    int i;
    Car *car = (Car *)car_ptr;
    sem_t *thread_sem;
    ...
    while (is_done == 0) {
        ...
        while ((current_time < car->arrival) ||
               (current_direction != car->direction && cars_on_bridge != 0) ||
               (current_direction == car->direction && cars_since_direction_change >= 5 && car->duration == car->duration_left && cars_on_bridge != 0)) {
            ...
            // Wait for time increment
            sem_wait(thread_sem);
        }

        sem_wait(&bridge_sem);
            // Change the current direction
            if (current_direction != car->direction) {
    ...
    while (fgets(line, line_len, stdin) != NULL) {
        ...
        for (token = strtok(line, " \t"); token != NULL; token = strtok(NULL, " \t")) {
            if (token_num == 0) {
                ...
                (*cars)[cars_count].direction = token[0];
            } else if (token_num == 2) {
